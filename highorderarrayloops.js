...
const arr = [1, 2, 3, 4, 5];
for (const i of arr) {
  //traversing through the array using for of loop
  console.log(i);
}
const str = "Hello Jishu";
for (const s of str) {
  //strings are also treated as arrays , here s refers to each element in the string
  console.log(s);
}
...
for (const [key, value] of map) {
  //[key,value ] it shows array destructuring
...
arrr.forEach((item, index, arr) => {
  //for Each has access to more than 1 param
...
myarrr.forEach((item) => {
  //using forEach we can print the values fetched from the object

  console.log(item.languageName);
});
//using filter in place of for Each method
//main difference is that filter returns a value and for each does not so we can only use for each for logging
//to return a value we need filter
const arr1 = myarrr.filter((item) => item.languageFileName === "js");
const arr2 = myarrr.filter((item) => {
  return item.languageFileName === "js";
}); //when we are definfing the scope of the callback using {} we have to use return for it.
//use filter when you want to define a condition and return something
console.log(arr1);
