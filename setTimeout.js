//setTimeout has trust issues,The timer you mention in setTimeout does no gurantee that it will execute the call back function after that timer expires everytime.
...
console.log("End");
//Supppose here there are millions line of code which needs atleast 10 sec to execute and time ris of 5 secs.
//what will js do?
...
//if gec is still getting execited in callstack it will not put the cb queue method in callstack.
...
//new Date()=returns the curren date it is Date api features.
...
Start
End
while expires(wait for 10 secs)
setTimeout calling
*/
...
//setTimeout with zero will also throw same behaviour becaus the callback function will go inside callback queue.
console.log("start");
setTimeout(function cb() {
    console.log("setTimeout calling")
}, 0);
...
setTimeout calling
*/
//if we have to defer some piece of code we can use this.
//means we want that piece of code to be executed later part in code .
